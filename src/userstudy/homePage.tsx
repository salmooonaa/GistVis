        ...
        <div style={{ display: 'flex', justifyContent: 'space-evenly', width: '100%' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <Link key={index} to={`/interactive/unprocessed/${index + 1}`} style={{ textDecoration: 'none', color: 'black' }}>
                    <button style={{ padding: '10px 20px', fontSize: '16px', margin: '5px', width: '100%' }} onClick={() => handleButtonClick(`Unprocessed_${index + 1}`)}>
                    {article}
                    </button>
                </Link>
                ))}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginLeft: '20px' }}>
                ...
                <Link key={index + 3} to={`/interactive/processed/${index + 7}`} style={{ textDecoration: 'none', color: 'black' }}>
                    <button style={{ padding: '10px 20px', fontSize: '16px', margin: '5px', width: '100%' }} onClick={() => handleButtonClick(`Processed_${index + 1}`)}>
                    {article}
                    </button>
                </Link>
                ))}
