...
import Extremechart from "./components/maxmin";
import "./page.css";
...
const ArticleEditor = ({ changeArticle, changeLlmArticle }) => {
  ...
  const submitUserInput = () => {
    ...
    LlmLink(userInput).then((message) => {
      console.log(message);
            ...
            return (
              <span key={part.id}>
                {parts1.map((item, index) => {
                  if (index % 2 === 1) {
                    return (
                      <span key={index} class="ano_value">
                        {item}
                      </span>
                    );
                  } else {
            ...
            return (
              <span key={part.id}>
                {parts.map((item, index) => {
                  if (index % 2 === 1) {
                    return (
                      <span key={index} class="spec_value">
                        {item}
                      </span>
                    );
                  } else {
...
export { ArticleWithImage, ArticleEditor, ArtcleProcess };
