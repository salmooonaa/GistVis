...
const line = ({options, myRef, type, centerValue}) => {
  ...
  const widthVis = type === 'sparkline' ? options.width - margin.left - margin.right :50;
  ...
  if (type === 'sparkline'){
    ...
    gChart.append('svg:defs').append('svg:marker')
      .attr('id', 'arrow')
...
const Sparkline = ({options, text}) =>{
  ...
  return (
    <span>
      ...
      <span style={{width: options.width, height:options.height }} ref={myRef}></span>
    </span>
  )
}
...
const Trend = ({options,text,type,centerValue}) =>{
  ...
  if (type==='negative-trend') {className = 'neg_text' }
  useEffect(() => {
