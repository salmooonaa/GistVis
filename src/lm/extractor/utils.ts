...
export const SpecDescriptions = {
  ...
  VALUE_VALUE_DESCRIPTION: {
    ...
    extreme:
      "The extreme (already converted into numbers).  If it does not exist or is uncertain, return NAN",
    proportion:
    ...
    trend:
      "The definition of the value of the data item according to the context. If it does not exist or is uncertain, return an empty string",
    value:
      "The numeric word(value). If it does not exist or is uncertain, return an empty string",
    ...
    dataSpec: z.array(
      z.object({
        ...
        categoryValue: z
          .string()
          .describe(
            SpecDescriptions.CATEGORY_VALUE_DESCRIPTION
          ),
        valueKey: z
          .string()
          .describe(
            SpecDescriptions.VALUE_KEY_DESCRIPTION
          ),
        valueValue: z
  ...
  const specWithPos = baseZodFormatting.extend({
    pos: z.array(
      z
        .string()
        .describe(
          "The words containing the value of extreme. Do not split the words if it only has one position."
        )
    ),
  });
