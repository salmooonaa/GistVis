...
const extrComp = async (model, textContent) => {
  ...
  const specParser = StructuredOutputParser.fromZodSchema(z.object({
    ...
    dataSpec: z.array(z.object({
      ...
      categoryValue: z.string().describe("The entity of the data item. Try to use phrases from the original text. If it does not exist, return an empty string"),
  ...
  const extrcompchain = RunnableSequence.from([
    PromptTemplate.fromTemplate(`
        ...
        Specifically, for 'categoryKey', identify the subject of comparison with its context, e.g. "the category of GDP growth" instead of just "entity". But the 'valueKey' of all data items should keep the same.
        For 'valueKey', specify the exact context of the value being compared, e.g. "the GDP growth rate" instead of just "value". But the 'category_key' of all data items should keep the same.
